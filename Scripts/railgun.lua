...
function Railgun:sv_modSwitch( index )
    self.network:sendToClients("cl_modSwitch", index)
end

function Railgun:cl_modSwitch( index )
...
function Railgun:cl_updateCol( index )
...
function Railgun:sv_explosion( pos )
    sm.physics.explode( pos, 7, 10, 20, 75, "PropaneTank - ExplosionBig" )
end
...
function Railgun:client_onFixedUpdate()
    if not self.tool:isLocal() then return end

    local tick = sm.game.getCurrentTick()
    if self.cl.ammo < 1 and tick - self.cl.shootTick >= self.rechargeTicks then
        self.cl.ammo = 1
    end
end
...
function Railgun.client_onEquip( self, animate )
	...
	if self.tool:isLocal() then
		-- Sets Railgun renderable, change this to change the mesh
...
function Railgun.cl_onPrimaryUse( self, state )
	if self.tool:getOwner().character == nil or state ~= sm.tool.interactState.start then
		return
	end
    ...
    if self.cl.modIndex == 1 then
        self.network:sendToServer("sv_pierce", sm.camera.getPosition())
    elseif self.cl.modIndex == 2 then
        local hit, result = sm.localPlayer.getRaycast( self.range * 10 )
        if hit then
            self.network:sendToServer("sv_explosion", result.pointWorld)
        end
    end
    ...
    self.cl.ammo = 0
    self.cl.shootTick = sm.game.getCurrentTick()
end

function Railgun.cl_onSecondaryUse( self, state )
    if state == sm.tool.interactState.null then return end
