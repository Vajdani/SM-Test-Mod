...
function StorageContainer.client_onInteract( self, character, state )
	if state == true then
		...
		if container then
			...
			elseif shapeUuid == obj_interactive_filecabinet or shapeUuid == sm.uuid.new("cb8e456e-a865-4ce8-aebe-d82d1129c5fa") then
...
function StorageContainer.client_onUpdate( self, dt )
	local shapeUUID = self.shape:getShapeUuid()
	if shapeUUID == sm.uuid.new("cb8e456e-a865-4ce8-aebe-d82d1129c5fa") or shapeUUID == obj_interactive_filecabinet or shapeUUID == obj_survivalobject_ruinchest then
