...
---@alias CharacterLiteralUUID
]]
	for k, set in pairs(characterSets) do
		for _k, character in pairs(sm.json.open(set).characters) do
			characterUUIDs = characterUUIDs..string.format("---| %s\n", ("`sm.uuid.new('%s')` # %s"):format(character.uuid, character.character.scriptClass))
		end
	end

	print(characterUUIDs)
end
